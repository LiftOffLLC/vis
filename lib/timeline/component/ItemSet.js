...
function ItemSet(body, options) {
    ...
    this.itemOptions = {
        //ngg-vis
        type: { start: "Moment", end: "Moment", JobEnd: "Moment" },
        // ngg-vis-end
    };
...
ItemSet.prototype.setOptions = function(options) {
    if (options) {
        ...
        ["onDropObjectOnItem", "onAdd", "onUpdate", "onRemove", "onMove", "onMoving", "onAddGroup", "onMoveGroup", "onRemoveGroup"].forEach(
            addCallback
        );
...
ItemSet.prototype.getVisibleGroups = function() {
    ...
    for (var groupId in this.groups) {
        if (this.groups.hasOwnProperty(groupId)) {
            var group = this.groups[groupId];
            if (group.isVisible && Number(groupId)) {
...
ItemSet.prototype._onDrag = function(event) {
    ...
    var cursorPos = event.center.x;
    var verticlCursrPos = event.center.y;
    var windowHeight = window.innerHeight;
    //ngg-vis end
    // to handle horizontal scroll ngg-vis
    if ((windowWidth <= cursorPos + 100 || cursorPos <= 250) && this.touchParams.itemProps) {
        ...
        if ((this.options.rtl && cursorPos <= 250) || (!this.options.rtl && windowWidth <= cursorPos + 100)) {
    ...
    if (this.touchParams.itemProps) {
        ...
        var updateGroupAllowed =
            ((this.options.editable.overrideItems || selectedItem.editable == null) && this.options.editable.updateGroup) ||
        ...
        this.touchParams.itemProps.forEach(
            function(props) {
                ...
                if (
                    ...
                    event.center.y > me.body.domProps.scrollTop &&
                    (verticlCursrPos + 150 >= windowHeight && !(verticlCursrPos > windowHeight))
                ) {
                    me.dragScrollTopOffset = 25;
                    me.body.domProps.scrollTop -= 50;
                } else if (event.additionalEvent === "panup" && scrollOffset < heightOffset && verticlCursrPos <= 250) {
                ...
                var updateTimeAllowed =
                    ((this.options.editable.overrideItems || selectedItem.editable == null) && this.options.editable.updateTime) ||
                    ...
                    if (props.dragLeft) {
                        ...
                        if (this.options.rtl) {
                            ...
                            if (itemData.end != undefined) {
                                ...
                                var x = event.center.x - container.offsetLeft;
                                itemData.end = new Date(me.body.util.toTime(x));
                        ...
                        if (itemData.start != undefined) {
                            ...
                            if (windowWidth <= cursorPos + 100 || cursorPos <= 250) {
                                var range = this.body.getWindow();
                            ...
                            var centerContainer = this.body.dom.centerContainer;
                            var xValue = event.center.x - centerContainer.offsetLeft;
                            // var y = event.center.x - util.getAbsoluteTop(container) + container.offsetTop;
                            var jobDuration = itemData.end.valueOf() - itemData.start.valueOf();
                            itemData.start = new Date(me.body.util.toTime(xValue));
                            ...
                            itemData.end = new Date(itemData.start.valueOf() + jobDuration);
            ...
            function(props) {
                ...
                if (!exists) {
                    ...
                    if (Number(itemData.group) && virtualObj) {
                        this._onDropObjectOnItem(event, itemData);
                    } else {
...
ItemSet.prototype._onGroupClick = function(event, nestedGrpId) {
    //ngg-vis
    var grpClass = event && event.target && event.target.getAttribute("class");
    if ((this.resourceSearchTerm && this.resourceSearchTerm.length > 0) || (grpClass && grpClass.indexOf("nesting-group") < 0)) {
        return;
    }
    //ngg-vis-end
    if (event) {
        ...
        var group = this.groups[nestedGrpId];
    } 
    var groupsData = this.groupsData.getDataSet();
    ...
    nestingGroup.showNested = event ? !nestingGroup.showNested : true;
 ...
 * Only called when `objectData.target === 'item'.
 * @param {Event} event The event
 * @param {dropObject} object
 * @private
 */
//ngg-vis
ItemSet.prototype._onDropObjectOnItem = function(event, dropObject) {
    var item = this.itemFromTarget(event);
    var objectData = dropObject ? dropObject : JSON.parse(event.dataTransfer.getData("text"));
