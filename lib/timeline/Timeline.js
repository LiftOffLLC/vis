...
var momentTimeZone = require("../module/momentTimeZone");
...
Timeline.prototype.setItems = function(items) {
    ...
    if (this.itemsData && items) {
        ...
        this.itemsData.update(updateItems);
...
//ngg-vis
Timeline.prototype.updateItems = function(items) {
    this.itemsData.update(items);
};
//ngg-vis-end
...
Timeline.prototype.setTimeZone = function(tz) {
...
//ngg-vis
Timeline.prototype.searchGroupData = function(term) {
    ...
    let groupsData = this.groupsData.getDataSet();
    let data = groupsData.get();
    let groups = data.filter(function(group) {
        return !group.nestedGroups;
    });
    let nestedGroups = data.filter(function(group) {
        return group.nestedGroups;
    });
    let groupsLen = groups.length;
    let nestedGroupsLen = nestedGroups.length;
    let nestedGroupsIdObj = {};
    for (var i = 0; i < groupsLen; i++) {
        let item = groups[i];
        if (
            new RegExp("^" + term, "i").test(item.Name) ||
            new RegExp(" " + term, "i").test(item.Name) ||
            (new RegExp("^" + term).test(item.id) && item.hasOwnProperty("nestedInGroup"))
        ) {
...
Timeline.prototype.highLightTech = function(id) {
    ...
    if (updateArr.length > 0) {
        const className = updateArr[0].className;
    ...
    if (id) {
        ...
        if (item) {
            ...
            var toPos = verticalScroll.itemTop - verticalScroll.scrollOffset;
...
Timeline.prototype.onGroupClick = function(id) {
    if(this.itemSet)  this.itemSet._onGroupClick(null, id);
}
//ngg-vis end
